...
var global$j = _global.exports = typeof window != 'undefined' && window.Math == Math ? window : typeof self != 'undefined' && self.Math == Math ? self // eslint-disable-next-line no-new-func
...
var is = isObject$s(document$2) && isObject$s(document$2.createElement);
...
(_redefine.exports = function (O, key, val, safe) {
  ...
  if (isFunction) has$a(val, 'name') || hide$6(val, 'name', key);
  if (O[key] === val) return;
  if (isFunction) has$a(val, SRC) || hide$6(val, SRC, O[key] ? '' + O[key] : TPL.join(String(key)));
...
var $export$1y = function $export(type, name, source) {
  ...
  for (key in source) {
    ...
    exp = IS_BIND && own ? ctx$8(out, global$g) : IS_PROTO && typeof out == 'function' ? ctx$8(Function.call, out) : out; // extend global
...
global$g.core = core$3; // type bitmap
...
var _objectKeysInternal = function _objectKeysInternal(object, names) {
  ...
  for (key in O) {
    if (key != IE_PROTO$2) has$9(O, key) && result.push(key);
...
var _setToStringTag = function _setToStringTag(it, tag, stat) {
  if (it && !has$8(it = stat ? it : it.prototype, TAG$1)) def(it, TAG$1, {
    configurable: true,
    value: tag
  });
};
...
$export$1f($export$1f.P + $export$1f.F * _fails(function () {
  if (html$1) arraySlice$2.call(html$1);
}), 'Array', {
...
var onFreeze = function onFreeze(it) {
  if (FREEZE && meta$5.NEED && isExtensible$1(it) && !has$6(it, META$1)) setMeta(it);
  return it;
};
...
if (!$Number(' 0o1') || !$Number('0b1') || $Number('+0x1')) {
  ...
  'EPSILON,isFinite,isInteger,isNaN,isSafeInteger,MAX_SAFE_INTEGER,' + 'MIN_SAFE_INTEGER,parseFloat,parseInt,isInteger').split(','), j$1 = 0, key$1; keys$1.length > j$1; j$1++) {
    if (has$4(Base$1, key$1 = keys$1[j$1]) && !has$4($Number, key$1)) {
  ...
  _redefine.exports(global$d, NUMBER, $Number);
}
...
$export$t($export$t.P + $export$t.R, 'Promise', {
  'finally': function _finally(onFinally) {
    var C = speciesConstructor$3(this, core$1.Promise || global$8.Promise);
...
/* , receiver */
  ...
  if (desc = gOPD$4.f(target, propertyKey)) return has$3(desc, 'value') ? desc.value : desc.get !== undefined ? desc.get.call(receiver) : undefined;
  if (isObject$7(proto = getPrototypeOf$2(target))) return get(proto, propertyKey, receiver);
}
...
if (_descriptors && (!CORRECT_NEW || _fails(function () {
  ...
  for (var keys = gOPN$2(Base), i$2 = 0; keys.length > i$2;) {
    proxy(keys[i$2++]);
  }
...
var _wksDefine = function _wksDefine(name) {
  var $Symbol = core.Symbol || (core.Symbol = global$6.Symbol || {});
...
var $defineProperty = function defineProperty(it, key, D) {
  ...
  if (has$1(AllSymbols, key)) {
    if (!D.enumerable) {
      if (!has$1(it, HIDDEN)) dP$1(it, HIDDEN, createDesc(1, {}));
      it[HIDDEN][key] = true;
    } else {
...
setToStringTag(global$5.JSON, 'JSON', true);
...
var ABV = !!(global$4.ArrayBuffer && global$4.DataView);
var CONSTR = ABV;
var i$1 = 0;
var l = 9;
var Typed;
...
while (i$1 < l) {
  if (Typed = global$4[TypedArrayConstructors[i$1++]]) {
...
if (_descriptors) {
  ...
  var validate$3 = function validate(it) {
  ...
  var proto$1 = {
    ...
    /* , thisArg */
      ...
      return speciesFromList(this, arrayFilter(validate$3(this), callbackfn, arguments.length > 1 ? arguments[1] : undefined));
    },
  ...
  var $setDesc = function defineProperty(target, key, desc) {
    if (isTAIndex(target, key = toPrimitive(key, true)) && isObject$2(desc) && has(desc, 'value') && !has(desc, 'get') && !has(desc, 'set') // TODO: add validation descriptor w/o calling accessors
...
var methods = {
  ...
  set: function set(key, value) {
    return weak$1.def(validate$1(this, WEAK_MAP), key, value);
  }
...
var enhance = function enhance($, uuid) {
  ...
  $('.assertion').each(function (index, element) {
    var node = $(element);
    var expectValue = node.text().trim();
    node.empty();
    node.attr('id', uuid());
    node.append("<span class=\"assert-expect\">".concat(expectValue, "</span>"));
...
var parse = function parse($, root, id, parseUtils, codes) {
  codes = codes || [];
  root.children().each(function (index, element) {
    ...
    if (node.hasClass('variable')) {
      ...
      var embeddedCode = parse($, node, id, parseUtils);
      parseUtils.parseFunction(node, codes, embeddedCode);
    } else if (node.hasClass('assertion')) {
      var _embeddedCode = parse($, node, id, parseUtils);

      parseUtils.parseAssertion(node, id, codes, _embeddedCode);
    } else {
...
var parseVariable = function parseVariable(node, codes) {
  var variableName = node.attr('data-name');
  var variableValue = node.text().trim();
  codes.push(convertVariableCode(variableName, variableValue));
};

var parseFunction = function parseFunction(node, codes, embeddedCode) {
  var actionName = node.attr('data-action');
  var actionParams = node.attr('data-params');
  codes.push(convertFunctionCode(actionName, actionParams, embeddedCode));
};

var parseAssertion = function parseAssertion(node, exampleId, codes, embeddedCode) {
  var actionName = node.attr('data-action');
  var actionParams = node.attr('data-params');
  codes.push(convertAssertionFunctionCode(actionName, actionParams, embeddedCode));
  var expectType = node.attr('data-expect');
  var expectValue = node.text().trim();
  codes.push(convertAssertionCode(expectType, expectValue));
  var assertionId = node.attr('id');
...
var convertAssertionResultCode = function convertAssertionResultCode(exampleId, assertionId) {
  return "$(\"#".concat(assertionId, "\").addClass(result ? \"success\" : \"error\");context[\"").concat(exampleId, "\"] = context[\"").concat(exampleId, "\"] && result;");
};
